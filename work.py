...
def create_slideshow_with_audio(image_paths, audio_path, output_path):
    if not image_paths or not os.path.exists(audio_path):
        print("❌ 이미지 또는 오디오 경로가 잘못되었습니다.")
        return

    if len(image_paths) != 1:
        print("❗ 현재는 단일 PNG만 지원합니다.")
        return

    image_path = image_paths[0]
    duration = get_mp3_duration(audio_path)
    temp_video = output_path.replace(".mp4", "_video.mp4")

    try:
        # 1단계: PNG → 임시 영상 (MP3 길이만큼 반복)
        ...
        # 2단계: 임시 영상 + MP3 → 최종 MP4 병합
        video_in = ffmpeg.input(temp_video)
        ...
        print(f"✅ 최종 MP4 병합 성공: {output_path}")

    except ffmpeg.Error as e:
        print("❌ ffmpeg 처리 중 오류 발생.")
        print("STDOUT:\n", e.stdout.decode(errors="ignore") if e.stdout else "없음")
        print("STDERR:\n", e.stderr.decode(errors="ignore") if e.stderr else "없음")
...
def generate_group_lesson(java_file_paths, group_path):
    ...
    save_text_as_image(all_code, png_path)

    # MP4 생성: PNG → 슬라이드 → MP3 병합
    output_mp4 = os.path.join(save_dir, filename + ".mp4")
    create_slideshow_with_audio([png_path], mp3_path, output_mp4)

    print(f"✅ 완료: {filename} → TXT, MP3, PNG, MP4 생성됨")
